package conectores;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import java.util.logging.Level;
import java.util.logging.Logger;
import modelos.Historial_Contrasenas;

public class Historial_Contrasenas_Controller extends Base_Datos
{
    public Historial_Contrasenas_Controller()
    {
        super();
    }

    /*
        --executeUpdate (para las modificaciones de una tabla) [Insert, Update, Delete]
        --executeQuery (para las consultas de una tabla) [Select]
        --execute (para las que modifican la estructura de la base de datos)
    */

    // Método para agregar un usuario
    public void add(Historial_Contrasenas HistContr)
    {
        Connection conn = super.getREF();
        
        // Verificar si la conexión es nula
        if (conn == null) 
        {
            System.err.println("Error: No se puede realizar la operación. Conexión a la base de datos no disponible.");
            return;
        }

        // Preparar la consulta SQL para insertar un nuevo usuario
        String sql = "Insert into HistorialContrasenas(HashContrasena, Salt, FechaUso" +
                     "values(?, ?, ?)";
        
        try (PreparedStatement ps = conn.prepareStatement(sql))
        {
            ps.setString(1, HistContr.getHashContrasena());
            ps.setString(2, HistContr.getSalt());
            ps.setObject(3, HistContr.getFechaUso());
            ps.executeUpdate();
            
            System.out.println("HistContr agregado correctamente");
        }
        catch (SQLException ex) 
        {
            Logger.getLogger(Usuario_Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}